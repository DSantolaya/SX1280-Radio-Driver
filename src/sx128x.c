...
static void sx128x_rx_internal_set(sx128x_t* dev, sx128x_rx_mode rx) {
  //LOG_DBG("internal set\n");
  switch (rx) {
    case sx128x_rx_receiving: //not executed
    LOG_DBG("set to rx_receiving\n");
      if (dev->settings.rx != sx128x_rx_listening) {
        LOG_ERR("[rx_state] Went from '%d' directly to 'receiving'\n", dev->settings.rx);
      }
      dev->settings.rx = sx128x_rx_receiving;
      break;
    case sx128x_rx_received:    //not executed
#if SX128X_BUSY_RX
...
sx128x_transmit(unsigned short payload_len) {
  ...
  ENERGEST_SWITCH(ENERGEST_TYPE_LPM,ENERGEST_TYPE_TRANSMIT);
  ...
  sx128x_set_standby(&SX128X_DEV);
  ENERGEST_SWITCH(ENERGEST_TYPE_TRANSMIT,ENERGEST_TYPE_LPM);
...
sx128x_pending_packet(void) {
  ...
  if (irq_reg & SX128X_IRQ_REG_RX_DONE) { 
    LOG_INFO("IRQ reg is RX \n");
    sx128x_cmd_get_packet_status(&SX128X_DEV);
    sx128x_cmd_get_rx_buffer_status(&SX128X_DEV);
    sx128x_set_state(&SX128X_DEV, SX128X_RF_IDLE);
    ENERGEST_SWITCH(ENERGEST_TYPE_LISTEN,ENERGEST_TYPE_LPM);
    sx128x_rx_internal_set(&SX128X_DEV, sx128x_rx_received);
    sx128x_cmd_clear_irq_status(&SX128X_DEV, SX128X_IRQ_REG_ALL);
...
sx128x_receiving_packet(void) {
#if SX128X_BUSY_RX  //executed
  if (SX128X_DEV.settings.rx == sx128x_rx_receiving) {  //Not executed Why not? is set in the return
    LOG_DBG("Rx recv \n");
  ...
  ENERGEST_SWITCH(ENERGEST_TYPE_LPM,ENERGEST_TYPE_CAD);
  uint16_t irq_reg = 0;
  while(!(irq_reg = sx128x_cmd_get_irq_status(&SX128X_DEV))); //Sale de este loop
  //LOG_INFO("interruption %d \n ",irq_reg) ;
  if (irq_reg & SX128X_IRQ_REG_CAD_DETECTED) { //NOT EXECUTED Never both conditions at the same time are T
  //LOG_INFO("Irq and CAD\n");
    sx128x_set_rx(&SX128X_DEV);
    ENERGEST_SWITCH(ENERGEST_TYPE_CAD,ENERGEST_TYPE_LISTEN);         
    sx128x_rx_internal_set(&SX128X_DEV, sx128x_rx_receiving);
  } else {
    sx128x_set_standby(&SX128X_DEV);  //So is set to standby and clear
    ENERGEST_SWITCH(ENERGEST_TYPE_LISTEN,ENERGEST_TYPE_LPM);
  }
  sx128x_cmd_clear_irq_status(&SX128X_DEV, SX128X_IRQ_REG_ALL);
#else
...
static void sx128x_gpio_init(sx128x_t* dev) {
  gpio_hal_arch_no_port_pin_cfg_set(dev->params.dio1_pin, GPIO_HAL_PIN_CFG_PULL_DOWN);//down
  gpio_hal_arch_pin_set_input(0, dev->params.dio1_pin);
  gpio_hal_arch_no_port_pin_cfg_set(dev->params.busy_pin, GPIO_HAL_PIN_CFG_PULL_DOWN);//down
  gpio_hal_arch_pin_set_input(0, dev->params.busy_pin);
  LOG_DBG("Gpio init completed\n");
...
static void sx128x_init_radio(sx128x_t* dev) {
  ...
  LOG_INFO("[sx128x] Get packet type: %d\n", sx128x_cmd_get_packet_type(&SX128X_DEV));
  LOG_DBG("Completed radio\n");
}
